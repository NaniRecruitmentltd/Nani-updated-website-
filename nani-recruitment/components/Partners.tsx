...
const partners: Partner[] = [
  ...
  },
  {
    id: 7,
    name: "DBS partner",
    description: "Marketing automation and customer engagement platform driving sustainable business growth through data-driven insights.",
    logo: "Magnit",
    logoImage: "/DBS R.png",
    color: "#ad4f68",
  },
...
const globePartners = partners.filter(p => p.id !== 8 && p.id !== 9);
...
const MatrixMagnitSlider = () => {
  ...
  const filteredPartners = partners.filter(p => p.id === 8 || p.id === 9);
      ...
      <div className="flex justify-center items-center mb-4 space-x-6">
        ...
        <div className="max-w-xl text-center">
          <AnimatePresence mode="wait">
            ...
            >
              <h3 className="text-xl font-bold mb-4" style={{ color: '#232d54' }}>
                {filteredPartners[currentIndex].name}
              </h3>
              <p className="leading-relaxed text-sm text-gray-600">{filteredPartners[currentIndex].description}</p>
            </motion.div>
          </AnimatePresence>
        </div>
...
export default function Partners() {
  ...
  return (
    <div className="min-h-screen bg-[#ECE4E8]">
      <div className="container mx-auto px-6 py-12 lg:py-20">
        ...
        >
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4" style={{ color: '#232d54' }}>
            Our Trusted Partners
          </h1>
          <p className="text-base lg:text-lg text-crimson -400 font-bold max-w-2xl mx-auto">
            Collaborating with industry leaders to deliver exceptional solutions and drive innovation forward
          </p>
        </motion.div>


      {/* Globe container WITHOUT white background, border, or shadow */}
      <div className="relative w-full h-[500px] max-w-md lg:max-w-none mx-auto rounded-3xl overflow-hidden">
        {globePartners.map((partner, i) => (
          <Globe key={partner.id} partner={partner} index={i} isMobile={isMobile} />
        ))}
      </div>

      {/* Slider showing only Matrix and Magnit */}
